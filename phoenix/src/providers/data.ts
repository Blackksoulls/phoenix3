...
export class Data {
  ...
  eradication(name) {
    ...
    Promise.all([dburlPromise,dbtablePromise,dbuserPromise,dbpassPromise,dbdbPromise]).then(([dburl,dbtable,dbuser,dbpass,dbdb])=> {
      ...
      this.http.post('https://phoenix3.srv1.rochodc.eu/API/delete.php', JSON.stringify(body), {headers: headers})
        .map(res => res.json())
        .subscribe(data => {
          if(data){
    ...
    if (data != undefined) {
      if ((data.textcontent == undefined) || (data.price == undefined) || (data.nb == undefined) || (data.datevalue == undefined)) {
        ...
        if (data.pic == undefined || data.pic == "") {
          data.pic = 'http://git.kuro.ml/sample.png';
        }
        ...
        Promise.all([dburlPromise,dbtablePromise,dbuserPromise,dbpassPromise,dbdbPromise]).then(([dburl,dbtable,dbuser,dbpass,dbdb])=> {
          ...
          this.http.post('https://phoenix3.srv1.rochodc.eu/API/addentry.php', JSON.stringify(body), {headers: headers})
            .map(res => res.json())
            .subscribe(data => {
              if(data == "1"){
      ...
      if (data != undefined) {
        if ((data.textcontent == undefined) || (data.price == undefined) || (data.nb == undefined) || (data.datevalue == undefined)) {
          ...
          Promise.all([dburlPromise,dbtablePromise,dbuserPromise,dbpassPromise,dbdbPromise]).then(([dburl,dbtable,dbuser,dbpass,dbdb])=> {
            ...
            this.http.post('https://phoenix3.srv1.rochodc.eu/API/editentry.php', JSON.stringify(body), {headers: headers})
              .map(res => res.json())
              .subscribe(data => {
